...
export function validateItem(item: Item | undefined, config: Config): Validation {
  ...
  if (item.availableFor && !item.availableFor.includes(config.model)) return { valid:false, reason:`${item.name} is not available for this model in the prototype catalogue.` };
...
export function normalizeConfigForModel(config: Config): Config {
  const wheels = find(catalog.wheels, config.wheels);
  const next = { ...config };
  if (!validateItem(wheels, next).valid) next.wheels = "19-aero";
  next.options = next.options.filter((id) => validateItem(find(catalog.options, id), next).valid);
  return next;
}
